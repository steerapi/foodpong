...
var BaseOrder, ConfirmCtrl, Discount, FirstCtrl, FoodPong, GraphsCtrl, IndexCtrl, LoginCtrl, ManageOrdersCtrl, ManageSubscriptionsCtrl, OrdersCtrl, PreOrders, RestaurantsCtrl, SettingsCtrl, SignupCtrl, capacity, createNewUser, dateTimeConverter, labor, listRestaurants, login, logout, r, randomUUID, timeConverter;
...
PreOrders = function(t) {
  return 3;
};

BaseOrder = function(time) {
  var orders;
  orders = void 0;
  if (time < 6) {
    return orders = Math.random() * 10;
  } else if (time < 12) {
    return orders = Math.random() * 30 + 20;
  } else if (time < 16) {
    return orders = Math.random() * 15 + 15;
  } else {
    return orders = Math.random() * 5 + 5;
  }
};

capacity = function(time) {
  var laborcap, maxcap;
  maxcap = 60;
  laborcap = 19;
  return Math.min(maxcap, labor(time) * laborcap);
};
...
Discount = function(time) {
  var capthresh, discinvrem, discount, discount_at, maxdiscord;
  capthresh = .6;
